...
export default function PortfolioScreen() {
  ...
  const handleAdd = async () => {
    ...
    try {
      await addToPortfolio({
        ...
        customName: selectedAsset === 'custom' ? customName : undefined,
        customCurrentPrice: selectedAsset === 'custom' ? Number(customCurrentValue) : undefined,
        currentValue: selectedAsset === 'custom' ? Number(customCurrentValue) : undefined,
      });
  ...
  const confirmDelete = async () => {
    if (itemToDelete) {
      await removeFromPortfolio(itemToDelete);
        ...
        <View style={styles.header}>
          <View style={styles.headerContent}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.headerSubtitle}>Yatırım Takibi</Text>
            </View>
          </View>
          <TouchableOpacity
            ...
            <ScrollView style={styles.modalBody}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.assetSelector}>
                {Object.keys(assetThemes).map((key) => {
                  if (key === 'minWage' || key === 'inflation' || key === 'stock' || key === 'car') return null;
                  ...
                  return (
                    ...
                    >
                      <Text style={[styles.assetChipText, isSelected && styles.assetChipTextSelected]}>
                        {key === 'btc' ? 'Kripto' : key === 'custom' ? 'Özel' : key.charAt(0).toUpperCase() + key.slice(1)}
                      </Text>
                    </TouchableOpacity>
                  );
                })}
              ...
              {selectedAsset === 'custom' && (
                <View style={styles.inputGroup}>
                  ...
                  <TextInput
                    ...
                    placeholder="Örn: Arsa, Antika, vb."
              ...
              <View style={styles.inputGroup}>
                <Text style={styles.label}>Yatırım Tutarı (TL)</Text>
                <TextInput
                  style={styles.input}
                  value={amount}
              ...
              {selectedAsset === 'custom' && (
                <View style={styles.inputGroup}>
                  <Text style={styles.label}>Güncel Değer (TL)</Text>
                  <TextInput
                    style={styles.input}
                    value={customCurrentValue}
                    onChangeText={setCustomCurrentValue}
                    placeholder="0.00"
                    keyboardType="numeric"
                    placeholderTextColor="#B0BEC5"
                  />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView contentContainerStyle={styles.modalList}>
              {dateStep === 'year' && Array.from({ length: new Date().getFullYear() - 2000 + 1 }, (_, i) => new Date().getFullYear() - i).map((year) => (
      ...
      >
        <View style={styles.deleteModalOverlay}>
          <View style={styles.deleteModalContent}>
            ...
            <Text style={styles.deleteTitle}>Yatırımı Sil</Text>
            <Text style={styles.deleteMessage}>
              Bu yatırımı portföyünüzden silmek istediğinize emin misiniz? Bu işlem geri alınamaz.
            </Text>
            <View style={styles.deleteActions}>
              ...
              >
                <Text style={styles.confirmButtonText}>Evet, Sil</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  deleteModalContent: { backgroundColor: '#fff', borderRadius: 24, padding: 24, width: '100%', maxWidth: 340, alignItems: 'center' },
  deleteIconContainer: { width: 64, height: 64, borderRadius: 32, backgroundColor: '#FFEBEE', alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
  deleteTitle: { fontSize: 20, fontWeight: '700', color: colors.text, marginBottom: 8 },
  deleteMessage: { fontSize: 14, color: colors.textSecondary, textAlign: 'center', marginBottom: 24, lineHeight: 20 },
  deleteActions: { flexDirection: 'row', gap: 12, width: '100%' },
  cancelButton: { flex: 1, paddingVertical: 14, borderRadius: 12, backgroundColor: colors.cardBackground, alignItems: 'center' },
  cancelButtonText: { fontSize: 16, fontWeight: '600', color: colors.text },
  confirmButton: { flex: 1, paddingVertical: 14, borderRadius: 12, backgroundColor: colors.danger, alignItems: 'center' },
